 ...
 * SplineIframeTest
 *
 * A COMPLETELY NEW TEST PAGE that uses a direct iframe embed for the Spline 3D background.
 * This is a standalone page that doesn't use any of the existing components.
 */
export default function SplineIframeTest() {
  const iframeRef = useRef<HTMLIFrameElement>(null);

  // Direct iframe embed URL
  const embedUrl = "https://prod.spline.design/EegKcAAn0DX9KgHh/embed";

  // Add error logging
  useEffect(() => {
    console.log("SplineIframeTest mounted");
    ...
    // Check if the iframe is accessible after a delay
    setTimeout(() => {
      try {
        if (iframe) {
          console.log("Iframe content accessible:", iframe.contentWindow !== null);
        }
      } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      {/* DIRECT IFRAME EMBED - ALWAYS VISIBLE */}
