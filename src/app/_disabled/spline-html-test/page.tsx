 ...
 * SplineHtmlTest
 *
 * A COMPLETELY NEW TEST PAGE that uses a direct HTML embed with the Spline viewer script.
 * This is a standalone page that doesn't use any of the existing components.
 */
export default function SplineHtmlTest() {
  ...
  const handleScriptLoad = () => {
    ...
    try {
      ...
      if (typeof customElements.get('spline-viewer') === 'undefined') {
        ...
        script.onload = () => {
          console.log("Manually loaded Spline script");
          setupViewer();
        };
        script.onerror = (error) => {
          console.error("Error loading Spline script manually:", error);
        };
  ...
  const setupViewer = () => {
    ...
    try {
      ...
      viewer.setAttribute('url', 'https://prod.spline.design/EegKcAAn0DX9KgHh/scene.splinecode');
      ...
      console.error("Error in setupViewer:", error);
    }
  };
